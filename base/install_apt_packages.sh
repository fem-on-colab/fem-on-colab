...
remove_packages_built_from_source () {
    grep -v -e "^gdal" -e "^libarmadillo" -e "^libboost" -e "^libgdal" -e "^libhdf" -e "^libhwloc" -e "^libkml" -e "^libnetcdf" -e "^libopenmpi" -e "^libvtk" -e "^mpi-default" -e "^openmpi" -e "^python-gdal" -h ${1} > ${1}.tmp
    mv ${1}.tmp ${1}
}
remove_packages_built_from_source ${COLAB_BACKEND_INFO}/apt-list-clean.txt

# Remove cuda packages to decrease the image size
remove_cuda_packages () {
    grep -v -e "^clinfo" -e "^cuda-" -e "^ffmpeg" -e "^libavcodec" -e "^libavdevice" -e "^libavfilter" -e "^libavformat" -e "^libavresample" -e "^libavutil" -e "^libchromaprint" -e "^libcublas" -e "^libcudnn8" -e "^libcufft" -e "^libcurand" -e "^libcusolver" -e "^libcusparse" -e "^libopencv" -e "^libncc" -e "^libnpp" -e "^libnvidia" -e "^libnvjpeg" -e "^libpostproc" -e "^libsw" -e "^libvdpau" -e "^libxnvctrl" -e "^nsight-compute" -e "^nsight-systems" -e "^nvidia" -e "^ocl" -e "^opencl" -e "^mesa-vdpau" -e "^vdpau-driver" -e "^xserver-xorg-video-nvidia" -h ${1} > ${1}.tmp
    mv ${1}.tmp ${1}
}
remove_cuda_packages ${COLAB_BACKEND_INFO}/apt-list-clean.txt
...
remove_R_packages ${COLAB_BACKEND_INFO}/apt-list-clean.txt
...
remove_java_packages ${COLAB_BACKEND_INFO}/apt-list-clean.txt
...
apt list --installed > ${COLAB_BACKEND_INFO}/apt-list-installed.txt
...
assert_removed_packages ${COLAB_BACKEND_INFO}/apt-list-installed.txt remove_packages_built_from_source
assert_removed_packages ${COLAB_BACKEND_INFO}/apt-list-installed.txt remove_cuda_packages
assert_removed_packages ${COLAB_BACKEND_INFO}/apt-list-installed.txt remove_R_packages
assert_removed_packages ${COLAB_BACKEND_INFO}/apt-list-installed.txt remove_java_packages
