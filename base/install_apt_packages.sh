...
remove_packages_built_from_source () {
    grep -v -e "^coinor" -e "^gdal" -e "^libarmadillo" -e "^libboost" -e "^libgdal" -e "^libhdf" -e "^libhwloc" -e "^libkml" -e "^libmumps"  -e "^libnetcdf" -e "^libopenmpi" -e "^libpmix" -e "^libscalapack" -e "^libscotch" -e "^libvtk" -e "^mpi-default" -e "^openmpi" -e "^python-gdal" -e "^python3-gdal" -h ${1} > ${1}.tmp
    mv ${1}.tmp ${1}
}
...
remove_cuda_packages () {
    grep -v -e "^clinfo" -e "^cuda-" -e "^ffmpeg" -e "^gds-tools" -e "^libavcodec" -e "^libavdevice" -e "^libavfilter" -e "^libavformat" -e "^libavresample" -e "^libavutil" -e "^libchromaprint" -e "^libcublas" -e "^libcudnn" -e "^libcufft" -e "^libcufile" -e "^libcurand" -e "^libcusolver" -e "^libcusparse" -e "^libopencv" -e "^libncc" -e "^libnpp" -e "^libnvidia" -e "^libnvfatbin" -e "^libnvjitlink" -e "^libnvjpeg" -e "^libpostproc" -e "^libsw" -e "^libvdpau" -e "^libxnvctrl" -e "^nsight-compute" -e "^nsight-systems" -e "^nvidia" -e "^ocl" -e "^opencl" -e "^mesa-vdpau" -e "^vdpau-driver" -e "^xserver-xorg-video-nvidia" -h ${1} > ${1}.tmp
    mv ${1}.tmp ${1}
}
...
apt install -y -qq autoconf autoconf-archive bison build-essential ca-certificates cmake curl flex git jq libtool libtool-bin ninja-build patchelf pkg-config rsync software-properties-common unzip wget
